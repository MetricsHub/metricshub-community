...
class ExplorerControllerTest {
	...
	@Test
	void testShouldReturnResources() throws Exception {
		final AgentTelemetry monitors = AgentTelemetry.builder().name("monitors").type("monitors").build();
		final AgentTelemetry connector = AgentTelemetry.builder().name("SNMP").type("connector").build();
		connector.getChildren().add(monitors);
		final AgentTelemetry connectors = AgentTelemetry.builder().name("connectors").type("connectors").build();
		...
		when(explorerService.getResources()).thenReturn(resources);

		mockMvc
			.perform(get("/api/resources"))
			.andExpect(status().isOk())
			.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
			.andExpect(jsonPath("$.name").value("resources"))
			.andExpect(jsonPath("$.type").value("resources"))
			.andExpect(jsonPath("$.children[0].name").value("server1"))
			.andExpect(jsonPath("$.children[0].children[0].name").value("connectors"))
			.andExpect(jsonPath("$.children[0].children[0].children[0].name").value("SNMP"))
			.andExpect(jsonPath("$.children[0].children[0].children[0].children[0].name").value("monitors"));
	}

	@Test
	void testShouldReturnEmptyResources() throws Exception {
		final AgentTelemetry resources = AgentTelemetry.builder().name("resources").type("resources").build();
		when(explorerService.getResources()).thenReturn(resources);
