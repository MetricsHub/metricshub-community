...
@SpringJUnitConfig(classes = { ToolCallbackConfiguration.class, SpringToolsToOpenAiToolsTest.TestConfig.class })
class SpringToolsToOpenAiToolsTest {
	...
	void testShouldCreateToolCallbackProvider() {
		...
		var tools = SpringToolsToOpenAiTools.buildOpenAiTools(toolCallbacks);

		assertNotNull(tools, "Tools should not be null");
		assertTrue(tools.size() > 0, "Tools should not be empty");
	}
