...
/**
 * MCP Tool Service for checking IPMI protocol reachability of a given host.
 */
@Service
public class IpmiProtocolCheckService {

	/**
	 * The type of the IPMI extension.
	 */
	private static final String IPMI_EXTENSION_TYPE = "ipmi";
	...
	@Autowired
	public IpmiProtocolCheckService(AgentContextHolder agentContextHolder, ProtocolCheckService protocolCheckService) {
		this.agentContextHolder = agentContextHolder;
		this.protocolCheckService = protocolCheckService;
	}

	/**
	 * Checks whether the specified host is reachable using the IPMI protocol.
	 *
	 * @param hostname the target host to check
	 * @param timeout optional timeout for the IPMI check in seconds
	 * @return a {@link ProtocolCheckResponse} indicating whether the host is reachable and how long the check took
	 */
	@Tool(name = "CheckIpmiProtocol", description = "Checks if a host is reachable using the IPMI protocol.")
	public ProtocolCheckResponse checkIpmiProtocol(
		@ToolParam(description = "The hostname to check") final String hostname,
		@ToolParam(description = "Timeout for the IPMI check in seconds", required = false) final Long timeout
	) {
