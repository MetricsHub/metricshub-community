	...
	public static class CreateCommand implements Callable<Integer> {
		...
		public Integer call() throws Exception {
			...
			try {
				...
				final var pwd = (password == null || password.length == 0) ? promptPassword() : new String(password);
				...
				final String bcryptHash = encoder().encode(pwd);
		...
		private static String promptPassword() throws Exception {
			...
			try {
				...
				final var s1 = new String(pwd1);
				final var s2 = new String(pwd2);
				if (!s1.equals(s2)) {
