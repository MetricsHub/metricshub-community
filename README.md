...
The upgrade policy is configured through the top-level `upgrade:` section (`enabled`, `allowDowngrade`, `hostAllowlist`, `maxPackageSizeBytes`, `downloadTimeout`, `downloadRetries`, `installTimeout`, `trustedCertificateFile`, `msiSignatureSubjectContains`). Archive (tar.gz/zip) and Docker deployments never accept package offers — container images are upgraded by redeploying the image.
