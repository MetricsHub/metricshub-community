...
* **metricshub-programmable-configuration-extension**: Provides a programmable configuration mechanism, allowing users to define custom configurations through [Apache Velocity](https://velocity.apache.org/) scripts. How whitespace around Velocity directives is removed from the generated YAML is controlled by the `parser.space_gobbling` JVM system property, set through the JVM options of the MetricsHub launcher (for example `-Dparser.space_gobbling=structured`). Accepted values are `none`, `bc`, `lines` and `structured`; it defaults to `lines`, which keeps lines made only of `#set`, `#if`, `#foreach` or `#end` directives from producing blank lines while preserving the indentation of the surrounding content. Any other value is rejected with a warning and falls back to `lines`.
