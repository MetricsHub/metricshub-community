...
import { fetchConfigList, fetchConfigContent, saveConfig, validateConfig, deleteConfig, renameConfig } from "../store/thunks/configThunks";
...
function ConfigurationPage() {
  ...
  useEffect(() => { dispatch(fetchConfigList()); }, [dispatch]);
  ...
  const submitDelete = useCallback(() => {
    if (!deleteTarget) { setDeleteOpen(false); return; }
      ...
      <Left>
        <Stack spacing={1.5} sx={{ p: 1.5 }}>
          <Stack direction="row" spacing={1} alignItems="center">
            <Button size="small" startIcon={<RefreshIcon />} onClick={() => dispatch(fetchConfigList())}>Refresh</Button>
