...
/** Rows above which pagination appears (the search field is always shown). */
...
const ProtocolHostnameOverridesTable = ({
	...
	// The snapshot holds row indexes, so it only survives edits confined to
	// override values: adding, removing, or reordering host.name entries shifts
	// the indexes and would make it show unrelated rows — drop it instead.
	...
	const paginated = hostNames.length > PAGINATION_THRESHOLD;
