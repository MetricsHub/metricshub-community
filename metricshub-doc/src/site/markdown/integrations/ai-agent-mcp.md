...
| `ExecuteHttpGetQueryService`     | <ul><li>`url` (list&lt;string&gt;, **required**)</li> <li>`headers` (string, _optional_)</li> <li>`body` (string, _optional_)</li> <li>`timeout` (integer, _optional_, default: *10s*)</li> <li>`poolSize` (integer, _optional_, default: *60 threads*)</li></ul>                       | Executes HTTP GET requests for one or more URLs, deriving hostnames from each target, and optionally includes custom headers, a request body, a timeout value, and concurrent execution settings.                                           |
