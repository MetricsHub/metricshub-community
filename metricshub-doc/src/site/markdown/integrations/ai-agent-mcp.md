...
| `ExecuteJmxQueryService`         | <ul><li>`hostname` (string, **required**)</li> <li>`objectName` (string, **required**)</li> <li>`attributes` (string, _optional_)</li> <li>`keyProperties` (string, _optional_)</li>  <li>`timeout` (integer, _optional_, default: *10s*)</li></ul>                                                  | Retrieves the specified MBean’s attributes and key properties from a target host using a JMX query, requiring at least one attribute or key property to be specified.                                         |
