...
| `hosts[]` | Monitored hosts, sorted by `resourceKey`. `hostnames` maps a protocol name (`ssh`, `snmp`, `wmi`, …) to the host name configured for it; `attributes` are the resource attributes |
